...
function detectIssuer(lines, filename){
  const txt = (lines.slice(0, 120).join(" ") + " " + filename).toUpperCase();
  if (/AMERICAN ?EXPRESS|AMEX/.test(txt)) return "AMEX";
  if (/CHASE|JPMORGAN/.test(txt)) return "VISA";
  if (/CITI|CITIBANK|THANKYOU/.test(txt)) return "CITI";
  return "CARD";
}

function inferYears(lines){
  const counts = {};
  const re = /\b(0?[1-9]|1[0-2])\/(0?[1-9]|[12]\d|3[01])\/(\d{2}|\d{4})\b/g;
